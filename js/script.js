  ...
  for (let i = 0; i < usageData.Type.length; i++) {
    $('.usage').append(
      `<tr>
        ...
        <td> ${usageData.core[i]}</td>
        <td> ${usageData.hdd[i]}</td>
        <td> ${usageData.flash[i]}</td>
  ...
  for (let i = 0; i < purchaseDataTable.id.length; i++) {
    $('.purchased').append(
      `<tr>
        ...
        <td> ${purchaseDataTable.type[i]}</td>
        <td> ${purchaseDataTable.capacity[i]}</td>
  ...
  for (let i = 0; i < multiClusterDataTable.id.length; i++) {
    $('.multi-cluster').append(
      `<tr>
        ...
        <td> ${multiClusterDataTable.unit[i]}</td>
...
$(document).ready(function() {
  ...
  $('.update-popup').click(() => {
    ...
    $('.add-node').click(() => {
      ...
      $('#fs').click();
      $('#sw').click();
      $('.aos-data:eq(1), .aos-data:eq(3),.aos-data:eq(5), .addon-data:eq(1), .addon-data:eq(2)').css({
        'opacity': '0.3',
        'pointer-events': 'none'
      });
